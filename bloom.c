 ...
 * https://github.com/8891689  gcc HashNyx_bloom.c bloom.c utils.c -O3 -o HashNyx_bloom -pthread -lm -static
 */
#define _GNU_SOURCE  
...
extern int hex_to_bytes(const char *hexstr, size_t hex_len, uint8_t *out_bytes, size_t out_size);
...
BloomFilter *bloom_init(uint64_t n, double p) {
    ...
    bf->filter  = calloc(bf->byte_count, 1); 
    if (!bf->filter) { free(bf); return NULL; } 
...
void bloom_free(BloomFilter *bf) {
    ...
    if (bf->mmaped) {
        ...
        size_t header_size = 7 + sizeof(bf->bit_count) + sizeof(bf->hash_count);
        munmap((char*)bf->filter - header_size, header_size + bf->byte_count); 
    } else {
        free(bf->filter); 
    }
#else
...
void bloom_reset(BloomFilter *bf) {
    pthread_rwlock_wrlock(&bf->lock);
    memset(bf->filter, 0, bf->byte_count); 
    pthread_rwlock_unlock(&bf->lock);
}
...
int bloom_save(const BloomFilter *bf, const char *filename) {
    ...
    if (write(fd, bf->filter, bf->byte_count) != (ssize_t)bf->byte_count) goto err; 
    close(fd);
    return 0;
err:
    close(fd);
...
BloomFilter *bloom_load(const char *filename) {
    ...
    tmp.filter  = malloc(tmp.byte_count); 
    if (!tmp.filter) { close(fd); return NULL; } 
    if (read(fd, tmp.filter, tmp.byte_count) != (ssize_t)tmp.byte_count) { 
        free(tmp.filter); close(fd); return NULL; 
    }
    close(fd);
...
int bloom_check_nolock(const BloomFilter *bf, const void *data, size_t len) {
    uint64_t h1 = hash1(data, len);
    uint64_t h2 = hash2(data, len);
    for (uint64_t i = 0; i < bf->hash_count; i++) {
        uint64_t idx = (h1 + i * h2) % bf->bit_count;
        if ((bf->filter[idx >> 3] & (1u << (idx & 7))) == 0) { 
            return 0;
...
int bloom_mmap_unload(BloomFilter *bf) {
    ...
    munmap((void*)( (char*)bf->filter - (7 + sizeof(bf->bit_count) + sizeof(bf->hash_count)) ), total);
    pthread_rwlock_destroy(&bf->lock);
    free(bf);
    return 0;
}
...
int bloom_add_hex(BloomFilter *bf, const char *hexstr) {
    uint8_t bin[128]; 
    int blen = hex_to_bytes(hexstr, strlen(hexstr), bin, sizeof(bin));
    if (blen < 0) return -1;
    bloom_add(bf, bin, (size_t)blen);
    return 0;
}
...
int bloom_check_hex(const BloomFilter *bf, const char *hexstr) {
    ...
    int blen = hex_to_bytes(hexstr, strlen(hexstr), bin, sizeof(bin));
    if (blen < 0) return -1;
    return bloom_check_nolock(bf, bin, (size_t)blen);
}
...
BloomFilter* bloom_init_optimal(uint64_t bit_count, int hash_count) {
    ...
    bf->hash_count = (uint64_t)hash_count; 

    bf->filter = (uint8_t*)calloc(bf->byte_count, sizeof(uint8_t));
    if (!bf->filter) { 
        free(bf);
        return NULL;
    }
